...
where $\hat{y}^c_i$  is the output of model and $y^c_i$ is the target that the model should return, $W_c$ is the weight of class $c$. Since the data set face the imbalanced problem then class weight for the loss is applied. In this research, both of the original weight and a new weight formula is implementation. Originally, the weight is calculated by taking the inverse of percentage that each class accounts for. The new weight formula is described as follow:
\[W = N \odot D\]
\[D = \begin{bmatrix}
